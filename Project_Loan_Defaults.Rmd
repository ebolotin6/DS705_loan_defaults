...
printRates <- function(ctable) {
  rate_list = c()
  true_negative <- round((ctable[1,1]/ctable[3,1])*100,2)
  true_positive <- round((ctable[2,2]/ctable[3,2])*100,2)
  # rate_list = c(rate_list, true_negative, true_positive)
  cat("TNR (%) - Recall: ", true_negative, "\n")
  cat("TPR (%) - Recall: ", true_positive, "\n")
  # return(rate_list)
}
```
...
ggplot(accuracy_plot, aes(x=t_list, y=y_list)) + 
  ...
  labs(title = "Model Accuracy vs Threshold", x = "Threshold", y = "Model Accuracy (%)") +
